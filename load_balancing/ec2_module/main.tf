...
resource "aws_instance" "clb_instance" {
  ...
  user_data = <<-EOF
              ...
              echo "<h1>Welcome to the EC2 instance in AZ: ${var.availability_zone}</h1><br><p>Instance Type: ${var.name}</p>" > /var/www/html/index.html
              EOF

  tags = {
    Name = "clb-instance-${var.availability_zone}"
  }
}
